//gcc -o divided_countchar_with_timers divided_count_char_ocurrences_using_pipes_with_timers.c && cat longFile.txt | ./divided_countchar_with_timers .

// correr 10 veces:
// (time -p perf stat -e task-clock,instructions,cache-references,cache-misses sh -c 'cat longFile.txt | ./divided_countchar_with_timers a') 2>> timePipes.txt

// (time -p perf stat -e task-clock,instructions,cache-references,cache-misses sh -c 'cat longloremipsum.txt | ./divided_countchar_with_timers a') 2>> timePipesLoremIpsum.txt

// (time -p perf stat -e task-clock,instructions,cache-references,cache-misses sh -c 'cat 124mbloremipsum.txt | ./divided_countchar_with_timers a') 2>> timePipes124mbLoremIpsum.txt
