 ...
 */
public class ejemploParaleloGiganteSeno extends Exception {
		...
		for (int i = 0; i < aRows; i++) { // aRow
			for (int j = 0; j < aColumns; j++) { // aColumn
				...
				finalResult[i][j]= finalResult[i][j] + 	(
														(int)(Math.sin(matrixA[i][index])
															*10)
														* 
														(int)(Math.sin(matrixB[index][j])
															*10)
														);
				}
			}
	...
	public static void main(String[] args) throws Exception{
		...
		ejemploParaleloGiganteSeno obj = new ejemploParaleloGiganteSeno();
