 ...
 */
public class ejemploParaleloGiganteCoseno extends Exception {
	...
	static void multiply(int index ) {
		...
		for (int i = 0; i < aRows; i++) { // aRow
			for (int j = 0; j < aColumns; j++) { // aColumn
				...
				finalResult[i][j]= finalResult[i][j] + 	(
														(int)(Math.cos(matrixA[i][index])
															*10)
														* 
														(int)(Math.cos(matrixB[index][j])
															*10)
														);
				}
			}
	...
	public static void main(String[] args) throws Exception{
		...
		threadNames=new String[] {t0.getName(), t1.getName(), t2.getName(), t3.getName()};
		...
		ejemploParaleloGiganteCoseno obj = new ejemploParaleloGiganteCoseno();
